...
def plot_reconstruction():
    ...
    # Codificação e decodificação com o autoencoder
    encoded_data = autoencoder.encoder(teste_data).numpy()
    decoded_data = autoencoder.decoder(encoded_data).numpy()
    
    # Convertendo para NumPy se necessário
    teste_data_np = teste_data.numpy() if hasattr(teste_data, 'numpy') else teste_data
    decoded_data_np = decoded_data.numpy() if hasattr(decoded_data, 'numpy') else decoded_data

    # Subtraindo as duas matrizes e pegando o valor absoluto
    result = teste_data_np - decoded_data_np
    result_abs = np.abs(result)
    
    # Somando os valores de cada coluna para obter um vetor 1D
    col_sums = np.sum(result_abs, axis=0)
    
    # Definindo o tamanho da janela da média móvel
    window_size = 1

    # Calculando a média móvel sobre o vetor col_sums
    moving_avg = np.convolve(col_sums, np.ones(window_size) / window_size, mode='valid')
    ...
    plt.plot(moving_avg, label='Média Móvel', color='blue')
    # Adicionando linha vertical vermelha em x=2000
    plt.axvline(x=2000-window_size, color='red', linestyle='--', label='Valor real')
    # Ajustando a legenda e rótulos
    plt.legend(labels=[f"Média Movel (j={window_size})", "Posição do dano"], loc=(0.5, 0.8), fontsize=28)
    plt.ylabel("Soma dos erros de reconstrução", fontsize=28)
    plt.xlabel("Posição (cm)", fontsize=28)
    plt.xticks(fontsize=26)
    plt.yticks(fontsize=26)
    plt.title('Média móvel das somas dos erros de reconstrução', fontsize=28)

    
    # Salvando a figura com alta qualidade
    plt.savefig(f'Media_Movel_{PosSensor}_{Teste_Caso}_Janela{window_size}.png', bbox_inches='tight', dpi=300)
...
def plot_ROCbasedKLD(ntestes):
    ...
    #encoded_data = np.zeros((n_vbatch, baseline_train.shape[0], latent_dim))
    ...
    metrica = DKL  #Media, Desvio, Mediana ou DI
    metric = 'DKL'
...
#ig, ax2  = plt.subplots()   #Ok
#encode_data = plot_ROCbasedKLD(ntestes=ncenarios)
